...
describe("String calculator should", () => {
  ...
  it(" return the total when given numbers separated by vertical bar separator.", () => {
    const stringCalculator = new StringCalculator()
    const result = stringCalculator.add("//|\n1|2|3")
  ...
  it(" return error when there is a separator other than the custom separator.", () => {
    const stringCalculator = new StringCalculator()
    expect(() => stringCalculator.add("//|\n1|2,3")).toThrowError(/^'\\|' expected but ',' found at position 3.$/)
  })

  it(" return error when there are negative numbers.", () => {
    const stringCalculator = new StringCalculator()
